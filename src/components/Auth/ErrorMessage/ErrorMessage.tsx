...
export type ErrorMessageProps = {
  ...
  /**
   * 서버메세지를 나타냅니다.
   * 서버에러가 있다면 서베 에러메세지를 string으로 넣어주면 됩니다.
   */
...
export function ErrorMessage({
  ...
  if (serverError) {
    return <p className={`${styles.error} ${className}`}>{serverError}</p>;
  }
  if (!name || !touched || !errors) return null;
  if (!touched[name] || !errors[name]) {
    return null;
  }
  return <p className={`${styles.error} ${className}`}>{errors[name]}</p>;
}
